...
####======================================================================DEFINE WINDOW==============================================================####
        ...
        self.root.title("Open Journal v.1.0.0")
...
####========================================================DATABASE_MANAGEMENT (SQLITE3)=========================================####
        ...
        def TradeData():#CREATE Table
            ...
            cur=con.cursor()
            cur.execute("CREATE TABLE IF NOT EXISTS Trades(id INTEGER PRIMARY KEY,Stock text,Date text,Hold text,Risk text,Allocation text,Outcome text,Play text)")
            con.commit()
            con.close()


        def addTrade(Stock,Date,Hold,Risk,Allocation,Outcome,Play):#ADD Entries to the Table
            con=sqlite3.connect("Trades.db")
            cur=con.cursor()
            cur.execute("INSERT INTO Trades VALUES (NULL,?,?,?,?,?,?,?)",(Stock,Date,Hold,Risk,Allocation,Outcome,Play))
            con.commit()
            con.close()
        ...
        def searchTrade(Stock="",Date="",Hold="",Risk="",Allocation="",Outcome="",Play=""):#RETRIEVE Rows Base on User Query
            con=sqlite3.connect("Trades.db")
            cur=con.cursor()
            cur.execute("SELECT * FROM Trades WHERE Stock=? OR Date=? OR Hold=? OR Risk=? OR Allocation=? OR Outcome=? OR Play=?",(Stock,Date,Hold,Risk,Allocation,Outcome,Play) )
            rows = cur.fetchall()   
            con.close()
            return rows
         
            
        def updateTrade(id,Stock,Date,Hold,Risk,Allocation,Outcome,Play):#CHANGE Row Entry(s)
            con=sqlite3.connect("Trades.db")
            cur=con.cursor()
            cur.execute("UPDATE Trades SET Stock=?, Date=?, Hold=?, Risk=?, Allocation=?, Outcome=?, Play=? WHERE id=?",(Stock,Date,Hold,Risk,Allocation,Outcome,Play,id,))
            con.commit()
            con.close()
...
####===================================================================BUTTON_ALGORITHMS====================================================####
        ...
        def AddTrade():##ADD row in User Database of Trades
            try:
                if(len(Stock.get())!=0):
                    addTrade(Stock.get(),Date.get(),Hold.get(),Risk.get(),Allocation.get(),Outcome.get(),Play.get())
        ...
        def SearchTrade():##SHOW rows from User Database of Trades
            ...
            if(len(Stock.get())!=0) or (len(Play.get())!=0):##SHOW only specified stock/play 
                for row in self.tree.get_children():
                    self.tree.delete(row)
                i=0
                for row in searchTrade(Stock.get(),Date.get(),Hold.get(),Risk.get(),Allocation.get(),Outcome.get(),Play.get()):
                    self.tree.insert('',i,text="",values=(row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7]))
                    i=i+1
        ...
        def Clear():##CLEARS Entry fields and Treeview
            Stock.set("")
            Date.set("")
            Hold.set(0)
            Risk.set(0)
        ...
        def TreeviewRow(a):##NOTE the ID of the Highlighted Row in Treeview
            try:
        ...
        def UpdateTrade():##REPLACE the Items in row in the Database with values in the Entry Field.
            ...
            try:
                updateTrade(RowID.get(),Stock.get(),Date.get(),Hold.get(),Risk.get(),Allocation.get(),Outcome.get(),Play.get())
                for row in self.tree.get_children():
                    self.tree.delete(row)
                i=0
                for row in viewTrades():
    ...
    #--------------------------------------------------------MATH FUCTIONS--------------------------------------------------#
        ...
        def MATH_AverageGain():
            ...
            tradeCount=len(IDlist)
            if tradeCount>0:
                averageGain=int(totalGain/tradeCount)
            else:
                averageGain=0
        ...
        def MATH_AverageLoss():
            ...
            tradeCount=len(IDlist)
            if tradeCount>0:
                averageLoss=int(totalLoss/tradeCount)
            else:
                averageLoss=0
...
####============================================================TREEVIEW======================================================####
        ...
        self.tree=ttk.Treeview(self.SearchFrame)##MAKE tkinter TREE
        self.tree.grid(row=0,column=0,sticky='nsew')
        self.tree['show']='headings'
        
        self.s=ttk.Style(self.SearchFrame)##DESIGN tree HEADING
        self.s.theme_use("clam")
        self.s.configure(".",font=('Helvetica',12))
        self.s.configure("Treeview.Heading",foreground=rose,font=('Helvetica',12,"bold"))
        
        ##DEFINE Tree heading
        self.tree["columns"]=("ID","STOCK","DATE","HOLD","RISK","ALLOCATION","OUTCOME","PLAY")
        self.tree.column("ID",anchor=CENTER,stretch=NO,width=125)
        self.tree.heading("ID",text="ID")
        self.tree.column("STOCK",anchor=CENTER,stretch=NO,width=160)
        self.tree.heading("STOCK",text="Stock")
        self.tree.column("DATE",anchor=CENTER,stretch=NO,width=160)
        self.tree.heading("DATE",text="Date")
        self.tree.column("HOLD",anchor=CENTER,stretch=NO,width=120)
        self.tree.heading("HOLD",text="Hold")
        self.tree.column("RISK",anchor=CENTER,stretch=NO,width=160)
        self.tree.heading("RISK",text="Risk")
        self.tree.column("ALLOCATION",anchor=CENTER,stretch=NO,width=160)
        self.tree.heading("ALLOCATION",text="Allocation")
        self.tree.column("OUTCOME",anchor=CENTER,stretch=NO,width=160)
        self.tree.heading("OUTCOME",text="Outcome")
        self.tree.column("PLAY",anchor=CENTER,stretch=NO,width=150)
        self.tree.heading("PLAY",text="Play")

        self.tree.bind('<ButtonRelease-1>',TreeviewRow)
        ...
        self.labelNOTHING.grid(row=7, column=0,columnspan=2)
 
        #Spacing
        self.labelNOTHING = Label(self.Left, text = "---------------------------------------------", font = ('Helvetica', 4, 'bold'), fg=gray,bg=blackcellphone )#DividingLine
        self.labelNOTHING.grid(row=8, column=0,columnspan=2)
        ...
        self.labelCAPITAL.grid(row=10, column=0, padx=padxBUTTON , pady=0 , sticky = 'W') 
        self.entryCAPITAL = Entry(self.Left, width= Left_width, font = ('Helvetica', 12, 'bold'),justify=CENTER,text=User_Capital)
        self.entryCAPITAL.grid(row=10 , column=1, padx=3, pady=2.5)

        #Spacing
        self.labelNOTHING = Label(self.Left, text = "",font = ('Helvetica', 4, 'bold'), bg=blackcellphone)#InvisibleTextforSpacing
        self.labelNOTHING.grid(row=11, column=0,columnspan=2)
        ...
        self.labelDEPOSIT.grid(row=12, column=0, padx=padxBUTTON , pady=0 , sticky = 'W') 
        self.entryDEPOSIT = Entry(self.Left, width= Left_width, font = ('Helvetica', 12, 'bold'),justify=CENTER,text=User_Deposit)
        self.entryDEPOSIT.grid(row=12 , column=1, padx=3, pady=2.5)

        #Spacing
        self.labelNOTHING = Label(self.Left, text = "",font = ('Helvetica', 4, 'bold'), bg=blackcellphone)#InvisibleTextforSpacing
        self.labelNOTHING.grid(row=13, column=0,columnspan=2)
        ...
        self.labelWITHDRAWAL.grid(row=14, column=0, padx=padxBUTTON , pady=0 , sticky = 'W') 
        self.entryWITHDRAWAL = Entry(self.Left, width= Left_width, font = ('Helvetica', 12, 'bold'),justify=CENTER,text=User_Withdrawal)
        self.entryWITHDRAWAL.grid(row=14 , column=1, padx=3, pady=2.5)
        ...
        #Spacing
        self.labelNOTHING = Label(self.Left, text = "",font = ('Helvetica', 4, 'bold'), bg=blackcellphone)#InvisibleTextforSpacing
        self.labelNOTHING.grid(row=17, column=0,columnspan=2)
        ...
        self.labelCAP.grid(row=18, column=0, padx=padxLABEL, pady=2.5, sticky = 'W')
        self.labelCAPValue = Label(self.Left, textvariable=Saved_Capital, font = ('Helvetica', 12, 'bold'), width = 10, fg=font_color,bg=darkgray )
        self.labelCAPValue.grid(row=18, column=1, padx=4, pady=2.5, sticky = 'W')
        ...
        self.labelEQ.grid(row=19, column=0, padx=padxLABEL, pady=2.5, sticky = 'W')
        self.labelEQValue = Label(self.Left, textvariable=User_Equity, font = ('Helvetica', 12, 'bold'), width = 10, fg=font_color,bg=darkgray)
        self.labelEQValue.grid(row=19, column=1, padx=4, pady=2.5, sticky = 'W')
